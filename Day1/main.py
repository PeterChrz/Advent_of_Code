...
### First Star
'''
for line in calibration_value:
    first_num = re.search(r'\d', line)
    second_num = re.search(r'\d(?=\D*$)', line)

    if first_num and second_num:
        line_total = int(first_num.group()) * 10 + int(second_num.group())
        grand_total += line_total

        print(f"Line total: {line_total}, Grand total: {grand_total}")
